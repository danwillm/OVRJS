...
#define CHECK_PROPERTY_ERROR(env, err)                                    \
  if (err) {                                                              \
    Napi::Error::New(env, vr::VRSystem()->GetPropErrorNameFromEnum(err)); \
    return env.Null();                                                    \
  }
...
Napi::Value IVRSystem::GetBoolTrackedDeviceProperty(const Napi::CallbackInfo& info) {
  ...
  bool result = vr::VRSystem()->GetBoolTrackedDeviceProperty(
      info[0].As<Napi::Number>().Int32Value(), static_cast<vr::ETrackedDeviceProperty>(info[0].As<Napi::Number>().Int32Value()), &err);
